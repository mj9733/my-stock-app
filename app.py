...
# 4. 팝업 함수 정의
# ==========================================
# [수정됨] 함수 이름 통일 (open_manager -> open_stock_manager)
@st.dialog("📋 종목 관리")
                ...
                try:
                    ...
                    for t in tickers:
                        try:
                            ...
                            items.append({"시간": dt.strftime("%m/%d %H:%M"), "종목": stock_name, "심리": status, "점수": score, "뉴스 제목": title, "링크": e.link})
                    ...
                    if items:
                        ...
                        c3.write("📈 **매우 탐욕**")
                        st.divider()
                        st.dataframe(pd.DataFrame(items), column_config={"점수": st.column_config.NumberColumn("강도", format="%d"), "링크": st.column_config.LinkColumn("원문", display_text="🔗")}, hide_index=True, use_container_width=True)
